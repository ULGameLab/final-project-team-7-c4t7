...
public class StormTrooperStateMachine : MonoBehaviour
{
    ...
    private int backToShooting = 5;
        //Patrol
    private Transform PatrolTarget,tempTarget,pTarget,pTarget1,pTarget2;
    ...
    public AnimationScript theState;
    ...
    void Start()
    {
        ...
        theState = this.gameObject.GetComponent<AnimationScript>();//animation States: Idle = 0,Walk = 1, Run = 2, Shoot = 3
    ...
    void enterSt()
    {
        ...
        Debug.Log(xRandom + " " + zRandom + " " + xNegative + " " + zNegative);
        Debug.Log(StrafeDestination);
    ...
    void executeP()
    {
        ...
        if (Mathf.Abs((transform.position - pTarget.transform.position).magnitude) < 1)
        {
            ...
            else if (pTarget == pTarget2) { pTarget = pTarget1;
                Debug.Log("2");
            }
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "PatrolTarget")
        {
            ...
            tempTarget = PatrolTarget.Find("Patrol1");
            onPatrol = true;
        }
    }
}
